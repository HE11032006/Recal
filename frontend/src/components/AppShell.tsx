...
const navItems = [
  { to: "/", label: "Aujourd'hui", icon: "radar" },
  { to: "/saved", label: "Sauvegardés", icon: "bookmark" },
  { to: "/profile", label: "Profil", icon: "person" },
];

export function AppShell() {
  const handleNew = useCallback((count: number) => {
    window.recal?.notify(
      "Recal",
      `${count} nouvelle${count > 1 ? "s" : ""} opportunité${count > 1 ? "s" : ""} identifiée${count > 1 ? "s" : ""} par ton agent.`
    );
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-surface">
      <div
        className="flex h-9 shrink-0 items-center border-b border-outline-variant/30 bg-surface pl-4 pr-32"
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-6xl px-margin py-space-xl">
            <Outlet />
          </div>
        </main>
      </div>
